...
# ---------------------------------------------------------------------------
# Modération — kick, ban, unban, mute, unmute, warn, clear
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Helpers JSON — sauvegarde des warns
# ---------------------------------------------------------------------------

def load_warns() -> dict:
    """Charge le fichier warns.json. Retourne un dict vide si inexistant."""
    try:
...
def save_warns(data: dict):
    """Sauvegarde le dict des warns dans warns.json."""
...
def add_warn(guild_id: int, user_id: int, moderator: str, raison: str) -> int:
    """
    Ajoute un warn pour un utilisateur dans un serveur.
    Retourne le nombre total de warns après ajout.
...
def get_warns(guild_id: int, user_id: int) -> list:
    """Retourne la liste des warns d'un utilisateur sur un serveur."""
    data = load_warns()
    return data.get(str(guild_id), {}).get(str(user_id), [])


def clear_warns(guild_id: int, user_id: int):
    """Supprime tous les warns d'un utilisateur sur un serveur."""
    data = load_warns()
    guild_key = str(guild_id)
    user_key = str(user_id)
...
class ModerationCog(commands.Cog):
    """Commandes de modération : kick, ban, unban, mute, unmute, warn, warns, clearwarns, clear."""
    ...
    async def send_sanction_log(self, guild: discord.Guild, description: str, color: discord.Color = discord.Color.orange()):
        """Envoie un embed de log dans le salon de sanctions dédié."""
    ...
    async def kick(self, interaction: discord.Interaction, membre: discord.Member, raison: str = "Aucune raison fournie"):
        if membre.top_role >= interaction.user.top_role and interaction.user != interaction.guild.owner:
            await interaction.response.send_message("Tu ne peux pas expulser ce membre (rôle égal ou supérieur au tien).", ephemeral=True)
            return
        try:
    ...
    async def ban(self, interaction: discord.Interaction, membre: discord.Member, raison: str = "Aucune raison fournie"):
        if membre.top_role >= interaction.user.top_role and interaction.user != interaction.guild.owner:
            await interaction.response.send_message("Tu ne peux pas bannir ce membre (rôle égal ou supérieur au tien).", ephemeral=True)
            return
        try:
    ...
    async def mute(self, interaction: discord.Interaction, membre: discord.Member, minutes: app_commands.Range[int, 1, 40320], raison: str = "Aucune raison fournie"):
        if membre.top_role >= interaction.user.top_role and interaction.user != interaction.guild.owner:
            await interaction.response.send_message("Tu ne peux pas muter ce membre (rôle égal ou supérieur au tien).", ephemeral=True)
            return
        try:
    ...
    async def warn(self, interaction: discord.Interaction, membre: discord.Member, raison: str = "Aucune raison fournie"):
        ...
        try:
            await membre.send(f"⚠️ Tu as reçu un avertissement sur **{interaction.guild.name}**. Raison : {raison}")
        except discord.Forbidden:
            pass
        await interaction.response.send_message(f"⚠️ {membre.mention} a été averti. Raison : {raison} (**{total} warn(s)** au total)")
        await self.send_sanction_log(
            interaction.guild,
            f"⚠️ **Warn** — {membre.mention} (`{membre.id}`)\nPar : {interaction.user.mention}\nRaison : {raison}\nTotal : **{total} warn(s)**",
            discord.Color.gold()
        )
    ...
    async def clearwarns(self, interaction: discord.Interaction, membre: discord.Member):
        ...
        await interaction.response.send_message(f"🗑️ {len(liste)} warn(s) supprimé(s) pour {membre.mention}.", ephemeral=True)
        await self.send_sanction_log(
            interaction.guild,
            f"🗑️ **Clearwarns** — {membre.mention} (`{membre.id}`)\nPar : {interaction.user.mention}\n{len(liste)} warn(s) supprimé(s)",
            discord.Color.blue()
        )
